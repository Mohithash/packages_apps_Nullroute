...
#
# Copy to vendor/bestrom/config/nullroute.mk and add exactly ONE line to
# vendor/lineage/config/common.mk:
#
#     include vendor/bestrom/config/nullroute.mk
#
# One include line means one merge conflict per Lineage rebase instead of N.
...
#
# ⚠ SCOPE. This runs at the moment vendor/lineage/config/common.mk includes it.
# Anything appended to PRODUCT_PACKAGES or PRODUCT_COPY_FILES *later* in the
# product-config pass is invisible to it. That is not a reason to skip it — it
# catches the common case where a vendor .mk is included before common.mk — but
# it is exactly why tools/ci_verify_image.sh re-asserts the same thing against
# the finished image, where nothing can hide. Run both.
...
# The stock file is 2 lines. Ours is ~2,000. If you still see 2, uncomment the
# block below — this file is included after Lineage's common.mk, so a
# PRODUCT_COPY_FILES entry here wins — and drop nullroute_etc_hosts from
# PRODUCT_PACKAGES above so the two mechanisms cannot fight.
#
# PRODUCT_COPY_FILES += \
...
# ---------------------------------------------------------------------------
#
#   * vendor/lineage/prebuilt/common/bin/50-lineage.sh must have its `etc/hosts`
#     entry deleted from the addon.d survival list. Otherwise the first OTA
#     restores the pre-OTA hosts file over the freshly built one and L0 silently
#     reverts — for the addon.d generation, not for this build, so it will pass
#     every check you run today and fail in a month.
#     tools/ci_verify_image.sh asserts this.
#
